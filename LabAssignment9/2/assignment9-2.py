...
def exp(rv):
    th = l2norm(rv)
    rv = normalized(rv)
    exp_matrix = np.array([[np.cos(th)+rv[0]*rv[0]*(1-np.cos(th)), rv[0]*rv[1]*(1-np.cos(th))-rv[2]*np.sin(th), rv[0]*rv[2]*(1-np.cos(th))+rv[1]*np.sin(th)],
                            [rv[1]*rv[0]*(1-np.cos(th))+rv[2]*np.sin(th), np.cos(th)+rv[1]*rv[1]*(1-np.cos(th)), rv[1]*rv[2]*(1-np.cos(th))-rv[0]*np.sin(th)],
                           [rv[2]*rv[0]*(1-np.cos(th))-rv[1]*np.sin(th), rv[2]*rv[1]*(1-np.cos(th))+rv[0]*np.sin(th), np.cos(th)+rv[2]*rv[2]*(1-np.cos(th))]])
    return exp_matrix
